...
export const read_identification_types = () => {
  const { data, error, isLoading, mutate } = useSWR(
    "/identification-types/all",
    fetcher
  );

  return {
    items: data
      ? data.map((x: IIdentificationType) => ({
          ...x,
...
export const create_identification_type = async (data: { name: string }) => {
  try {
    const res = await API.post("/identification-types/", data);
    // console.log(res);
...
export const delete_identification_type = async (id: number) => {
  try {
    const res = await API.delete(`/identification-types/${id}`);
    // console.log(res)
    return callResult(res, res.data);
  } catch (err) {
    return axiosError(err);
...
export const update_identification_type = async (
  ...
  try {
    const res = await API.put(`/identification-types/${id}`, data);
    return callResult(res, res.data);
  } catch (err) {
    return axiosError(err);
  }
...
export const read_identification_type = async (id: number) => {
  try {
    const res = await API.get(`/identification-types/${id}`);

...
import {
  create_fire_peril_class,
  update_fire_peril_class,
...
const Editor: React.FC<EditorProps> = ({
  ...
  const handleSubmit = async (values: Record<string, any>) => {
    try {
      setBusy(true);
      const res = edit
        ? await update_fire_peril_class(id, {
            name: values.name,
            code: values.code,
          })
        : await create_fire_peril_class({
            name: values.name,
            code: values.code,
          });
      ...
      if (res.success) {
        showSuccess(
          edit
            ? "Successfully updated peril class"
            : "Successfully created peril class"
        );
        isDone();
      } else {

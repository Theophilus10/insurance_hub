...
const Page = () => {
  ...
  const [data, setData] = useState<IShippingType | null>(null);
  ...
  const { items, isError, isLoading, mutate } = read_shipping_types();
  useEffect(() => {
    if (isError) {
  ...
  const handleDelete = async (id: number) => {
    try {
      setBusy(true);
      const res = await delete_shipping_type(id);
      if (res.success) {
  ...
  return (
    <div className="flex flex-col w-full h-full gap-4 pt-2">
      ...
      <DataTable
        ...
        columns={columns}
        addButtonLabel="Add Shipping Type"
      ...
      <Modal
        open={openModal}
        size="sm"
        title={edit.open ? "Update Shipping Type" : "New Shipping Type"}

...
import { ICountry, delete_country, read_countries } from "@app/server/services";
...
const Page = () => {
  ...
  const { items, isError, isLoading, mutate } = read_countries();
  useEffect(() => {
    if (isError) {
  ...
  const handleDelete = async (id: number) => {
    try {
      setBusy(true);
      const res = await delete_country(id);
      if (res.success) {
  ...
  return (
    <div className="flex flex-col w-full h-full gap-4 pt-2">
      ...
      <Modal
        open={openModal}
        size="sm"
        title={edit.open ? "Update Country" : "New Country"}
